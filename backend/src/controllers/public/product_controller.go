...
func GetProductDetailBySlug(c *gin.Context) {
	...
	hostURL := helpers2.BuildHostURL(c)
	productResponse := structs2.ToProductResponseWithBaseURL(product, hostURL)
	...
	if tokenString != "" {
		...
		token, err := helpers2.VerifyToken(tokenString) // We need to expose VerifyToken in helpers
