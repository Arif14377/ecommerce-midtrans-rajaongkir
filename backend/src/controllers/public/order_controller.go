...
func Checkout(c *gin.Context) {
	// ambil body request
	var req structs2.CheckoutRequest
	...
	var orderItems []models2.OrderItem
	...
	for _, cart := range carts {
		...
		orderItems = append(orderItems, models2.OrderItem{
			OrderId:   orderID,
	...
	token, redirectURL, err := helpers2.GetSnapToken(order, user)
	if err != nil {
		tx.Rollback()
	...
	if err := tx.Where("user_id = ?", userID).
		Delete(&models2.Cart{}).Error; err != nil {
	...
	c.JSON(http.StatusOK, structs2.SuccessResponse{
		...
		Data: structs2.CheckoutResponse{
			SnapToken:   token,
