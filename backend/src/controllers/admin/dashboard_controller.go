...
func Dashboard(c *gin.Context) {
	...
	database.DB.Model(&models2.Order{}).Select("sum(total_price) as total").Where("status = ?", "paid").Scan(&revenueResult)
	totalRevenue = revenueResult.Total

	if err := database.DB.Model(&models2.Order{}).Count(&totalOrders).Error; err != nil {
	...
	database.DB.Model(&models2.Order{}).Where("status = ?", "pending").Count(&pendingOrders)
	database.DB.Model(&models2.Order{}).Where("status = ?", "paid").Count(&paidOrders)

	if err := database.DB.Model(&models2.Product{}).Count(&totalProducts).Error; err != nil {
		c.JSON(http.StatusInternalServerError, structs2.ErrorResponse{
			Success: false,
	...
	var recentOrders []models2.Order
	database.DB.Preload("User").Order("created_at desc").Limit(5).Find(&recentOrders)

	// Map ke Struct Response
	var recentOrderResponses []structs2.OrderResponse
	for _, o := range recentOrders {
		recentOrderResponses = append(recentOrderResponses, structs2.OrderResponse{
			Id:        o.Id,
	...
	c.JSON(http.StatusOK, structs2.SuccessResponse{
		...
		Data: structs2.DashboardResponse{
